...
export const TRACKER_CACHE_VERSION = 'pose-yolo-source-modes-v1';
...
function normalizeBallSource(source) {
  const raw = String(source || '').toLowerCase();
  if (raw === 'hough' || raw === 'yolo' || raw === 'yolo_fallback_hough') return raw;
  return 'hough';
}

function normalizeRacketSource(source) {
  const raw = String(source || '').toLowerCase();
  if (raw === 'pose' || raw === 'yolo' || raw === 'yolo_fallback_pose') return raw;
  return 'yolo_fallback_pose';
}
...
export async function inspectTrackCache(videoPath, options = {}) {
  ...
  const expectedBallSource = normalizeBallSource(config?.tracking?.objectBallSource);
  const expectedRacketSource = normalizeRacketSource(config?.tracking?.objectRacketSource);
  ...
  if (config?.tracking?.objectDetectorEnabled !== false && normalizeBallSource(objectMeta?.ballSource) !== expectedBallSource) {
    return { ok: false, reason: 'object_ball_source_mismatch' };
  }
  if (config?.tracking?.objectDetectorEnabled !== false && normalizeRacketSource(objectMeta?.racketSource) !== expectedRacketSource) {
    return { ok: false, reason: 'object_racket_source_mismatch' };
  }
  if (
...
async function runPythonObjectDetector(videoPath, sidecarPath, options = {}) {
  ...
  const ballSource = normalizeBallSource(config?.tracking?.objectBallSource);
  const racketSource = normalizeRacketSource(config?.tracking?.objectRacketSource);
  const args = [
    scriptPath,
    '--video',
    videoPath,
...
async function generateTracks(videoPath, sidecarPath, options = {}) {
  ...
  try {
    ...
    existing.objectDetectorMeta = {
      ...
      ballSource: normalizeBallSource(config?.tracking?.objectBallSource),
      racketSource: normalizeRacketSource(config?.tracking?.objectRacketSource),
      error: err.message
    };
    existing.trackerMeta = {
      ...
      objectBallSource: normalizeBallSource(config?.tracking?.objectBallSource),
      objectRacketSource: normalizeRacketSource(config?.tracking?.objectRacketSource)
    };
...
export async function getOrCreateTracks(videoPath, options = {}) {
  ...
  if (await fileExists(sidecarPath)) {
    ...
    const expectedBallSource = normalizeBallSource(config?.tracking?.objectBallSource);
    const expectedRacketSource = normalizeRacketSource(config?.tracking?.objectRacketSource);
    const matchesObjectConfig = config?.tracking?.objectDetectorEnabled === false
      || (
        String(objectMeta?.model || '') === expectedObjectModel
        && normalizeBallSource(objectMeta?.ballSource) === expectedBallSource
        && normalizeRacketSource(objectMeta?.racketSource) === expectedRacketSource
      );
